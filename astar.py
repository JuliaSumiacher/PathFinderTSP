...
class AStarSearch:
    ...
    def search(grid: Grid) -> Solution:
        ...
        frontier.add(node, node.cost + heuristic(node.state, grid))
        ...
        while not frontier.is_empty():
            ...
            for action, state in successors.items():
                ...
                # Check if the successor is not reached or if it has been reached with a greater cost
                if state not in explored or cost < explored[state]:
                    ...
                    frontier.add(son_node, son_node.cost + heuristic(son_node.state, grid))
